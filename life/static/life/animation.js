...
// ANIMATION SYNTAX: in the CSS styling it's 'stroke-dasharray', in JS for the animation function (element.animate) it's strokeDasharray, as CSS attributes are usually described in JS. It doens't take strings as names like 'stroke-dasharray'. Multiple values are in inverted comma: 'number, number, ...' not in array, like [1,3]. This in contrast to setAttribute for SVGs which does take strings as names and values in array brackets: shape.setAttribute('stroke-dasharray', [number, number])
// TIP: create shapes in Illustrator or another vector program, save as SVG and open it in a text editor. Edit the file in the vector graphics program to see on the fly how the SVG is constructed in the text editor. You can directly copy it over to your HTML space to take effect in your app.
...
const contractXToCenter =
[
	{transform: 'scaleX(1)'},
	{transform: 'scaleX(0.01)'}
]
const expandXFromCenter =
[
	{transform: 'scaleX(0.01)'},
	{transform: 'scaleX(1)'}
]
...
{
	return [
		{ backgroundColor: 'hsl('+colorDegree+', 70%, 70%)'}, // it is already at 70% 70% from mouse hovering
		{ backgroundColor: 'hsl('+colorDegree+', 85%, 85%)'},
		{ backgroundColor: 'hsl('+colorDegree+', 70%, 70%)'}
	]
}
...
{
	return [
		{backgroundColor: 'hsl('+colorDegree+', 50%, 50%)'},
		{backgroundColor: 'hsl('+colorDegree+', 70%, 70%)'}
	]
}
...
function removeAni (initialColor, deleteSignalColor)
{
	return [
		...
		{backgroundColor: initialColor},
		{backgroundColor: initialColor},
		{backgroundColor: deleteSignalColor}
	]
}

const fadeTime = 333;
const timing = {duration: fadeTime};

export {svg, svg2, fadeTime, timing, fadeIn, fadeOut, imageFadeIn, imageFadeOut, contractToLeft, expandFromLeft, contractXToCenter, expandXFromCenter, fontOpacityZeroToFull, brighten, brightenHover, removeAni};