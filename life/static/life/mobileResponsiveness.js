...
// Originally this module was supposed to give interactive features to the fullWindow mode (full-screen images) on smartphones and tablets, with one functionality being that you can pull arrows towards the center to navigate to the next / previous image (like canva presentations on mobiles). But the fullWindow mode itself proved tricky for mobiles. It had especially to do with the different zoom level and the fact that you could tilt the screen, where it didn't get the position of the fullWindow perfectly right (on a potentially 30k or more pixel high document) I tried implementations and a lot of testing, and it mostly worked, but in the end I just realized that the safest and most sensible would be to add a big screen presentation to the GBIFResult at the bottom, which solved everything. In the GBIFResult object it's called mobileFullWidthImage. If the user has the smartphone upright (like aspect ratio of > 0.5) and it loads a widescreen image (aspect ratio of 2 or more), they can just tilt the phone and there it perfectly is, filling the screen, at the lower end of the GBIFResult space.
// Along the way I also ditched Bootstrap because the mobile adjustment took too much out of my hands without me exactly knowing what was going on. For debugging on mobiles I had to eliminate any potential influence. Eventually I figured I didn't need Bootstrap, so I did everything by hand, which wasn't much really. I just needed to apply a zoom on the body (different zooms for either smartphone tilt) and make some style adjustments in the mobile.css file, which is a copy of styles.css with small-screen adjustment.
// It was only after I abandoned (after a long fight) the fullWindow mode for mobiles, that I realized that window.srollTo didn't have a zoom multiplier. That might have solved it. If you have a zoom of 1.5, you need to add that as a multiplier to window.scrollTo and window.scrollBy. Nontheless, I like this solution much more, it's much more mobile friendly and lovely in look. The fullWindow mode is really more fitting for desktops. I'm still grateful for this exploration as it taught me a lot about the touch gestures. For e.g. the arrow-pull navigation (like in Canva) worked perfectly fine. And it was intuitive too, I just needed to learn about the (fairly low-level) touch gestures on MDN a bit and could then quickly implement a gesture completely by my own reasoning.
