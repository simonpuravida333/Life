...
export default function fetchEverything(GBIFResult, y)
{
	...
	// originally all these JS modules were one file, before I eventually split them up. But suddenly JS would not accept this way of declaring variable anymore: let childrenLoaded = synonymsLoaded = distributionsLoaded = vernacularNamesLoaded = false. It seems declrations have to be more explicit within modules. Also the for-in loops needed updating from 'for (index in list)' to 'for (let index in list)' (later replaced for-ins with for-ofs).
	...
	.then(incoming =>
	{
		...
		if (y < 6 && incoming.results.length !== 0) // there are in fact sometimes GENEA, FAMILIES etc. that don't have children; it's the rarer case, but it exists. 
		{
			...
			theInfo.appendChild(superElement);
			superElement.animate(fadeIn, fadeTime);
			allSuperElements.push(superElement);
			if (superElement.getBoundingClientRect().width > 600) superElement.style.width = '600px'; // why getBoundingClientRect() and not clientWidth? Because client-width only reads out the set styling of the div element, while getBoundingClientRect reads the actual rendering on the screen. In this case clientWidth would just be nothing, as no width is set. Oftentimes dimensions are dependent on other objects, often parent objects, as it is in this case. If one of the superElements that comprises the whole fetch-content, meaning if one of the returns from one fetch, e.g. 'LOCALITY' has a one-line string that is long, it will cause superElement and thus the parent object 'info' to be stretched across the window, which will cause it to also stretch the other children (other superElement from other fetches). But I want to allow that if it is less than 600px, it should use less, otherwise it would look silly; it has to be dynamic. Sometimes all the gathered species description (content of 'info') is like just two fetches (superElements) with two words each.
		}
		childrenLoaded = true;
	})
	...
	.then(incoming =>
	{
		...
		if (incoming.results.length !== 0)
		{
			...
			{ 
				...
				if (name.language !== 'eng' && name.language !== '') continue;
